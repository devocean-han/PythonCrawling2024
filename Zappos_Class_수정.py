...
class ZapposSoupTest(bs):
	...
	'''
	def __init__(self, url, isTesting=False, size_type=2):
		self.url = url
		...
		def get_webpage_response(url, headers):
			...
			if response.status_code == 200:
				...
				print('Error happend with HTTP response----------')
	...
	def set_options_quantity(self):
		''' M, L 처럼 옵션이 숫자가 아닌 경우, 50으로 일괄 설정하기
		 방법1) self.size_type이 2면 무조건 영문(숫자 아님)으로 취급하고 따로 설정
		  방법2) 그냥 self.options.size[0]가 숫자인지 검사해서 아니면 따로 설정 '''
		min_final_size = self.options_size[0]
		if not (str(min_final_size)).isdigit(): 
			self.options_quantities = [self.options_quantity for size in self.options_size]
		else:
			self.options_quantities = self.options_size
...
class GoogleSheetsTest():
	...
	def set_ZapposClass(self, url, isTesting=True, size_type=2):
		self.z = ZapposSoupTest(url, isTesting, size_type)
...
def main():
	...
	while True:
		...
		# 자포스 클래스 생성
		g.set_ZapposClass(g.url, isTesting=False, size_type=int(product_type))
